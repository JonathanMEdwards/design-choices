...
\authorinfo{is a post-doctoral researcher at Charles University interested in
overcoming pervasive ``premature commitment'' in programming. His PhD at
University of Kent was about creating programming systems that combine
self-sustainability and notational freedom.
Send him an e-mail at \email{jakubovic@d3s.mff.cuni.cz} and read his blog at
\href{https://programmingmadecomplicated.wordpress.com}{\texttt{programmingmadecomplicated.wordpress.com}}.
 }
...
Those \emph{document-oriented programming systems} have a specific set of design choices. The
...
full state is managed by the underlying hidden kernel.}
\label{fig:systems}
\end{figure}
...
traced to the 1980s \cite{disessa-1986-boxer}. The paradigm has a rich history, but there has also
...
As the examples of document-oriented programming systems illustrate,
...
how rich user interfaces are embedded within the document, how programming is integrated
with the document and also how computations are executed.

We argue that to advance research on document-oriented programming systems, we need to precisely
characterise what a document-oriented programming system~is, understand the design
choices available to the designers of document-oriented programming systems, and
...
dimensions, the \emph{staging of customization} is such that the modification of the
...
\item We review a number of document-oriented programming systems to provide reference points
  for discussion about system design (Section~\ref{sec:examples}).
...
\item We illustrate one of the uses of our catalogue (Section~\ref{sec:intro-using}) by
  identifying four interesting under-explored combinations of design choices (Section~\ref{sec:directions}).
\end{itemize}
...
the system was the writing of interactive textbooks. The readers of such textbook would be able
...
{\scshape Data} boxes contain text, graphics and other boxes. {\scshape Doit} boxes contain program
text, other {\scshape Doit} and {\scshape Data} boxes \cite[p.2]{klotz-1989-boxer}. Boxes are used
to represent structured data (such as a collection of records) as well as nesting structure
in code (body of a loop). The system also includes {\scshape Graphics} boxes that can display
...
Computation in Boxer is represented as code in {\scshape Doit} boxes. The code can be executed by
...
it also produces a result, the result is placed in a new {\scshape Data} box in the document.
...
\subsection{Commercial Success: HyperCard, Spreadsheets and Jupyter}
Boxer illustrates many of the key aspects of document-oriented programming systems, but it
is not widely known. However, there has also been a number of systems that are based on a
less interesting combination of design choices, but were widely used.

\paragraph{HyperCard.}
In HyperCard (Figure~\ref{fig:hyper}), the document is a stack of cards that contain graphics,
text and interactive elements. As usual for document-oriented programming systems, both the
editing and using of programs is done through the same document interface. What the user can do
with a document is determined by the current user level, ranging from Browsing (just viewing)
to Painting (change text and graphics) and Scripting (specify actions, write code).

Simple interactivity can be implemented by linking buttons to cards (when a button is clicked,
the specified card is displayed). Further logic can be implemented by writing scripts in the
HyperTalk programming language. Scripts can be attached to document elements, they
are imperative and can modify the document. In particular, they can create and delete objects
and change their properties. Unlike in Boxer, HyperTalk scripts are represented as textual
properties of elements. They are not normally displayed as part of a card and are edited through
a text editor in a separate window.

HyperCard exhibits the key characteristics of a document-oriented programming system. The user
works with it through a document structure (stack of cards) and they can modify program while using
it. HyperTalk is a text-based scripting language, but scripts can access and modify the document.

\paragraph{Spreadsheets.}
Spreadsheets are a prime example of a successful end-user programming system \cite{nardi-1993-small}.
Like HyperCard, they satisfy the criteria for a document-oriented programming system.
The document structure is a grid of cells and the user edits and runs the programs they create
through the document interface.

Unlike in HyperCard or Boxer, evaluation in spreadsheets is immediate and the computation
results update as soon as data or a formula is modified. The basic programming model is based
on a formula language. Formulas can refer to other cells (using a relative or an absolute location
reference) and produce results that are displayed in the spreadsheet. Formula evaluation does not
modify the document. The system displays the results in the cells, but the underlying document
remains unchanged (i.e., a formula in a cell is not \emph{replaced} with the result).
A range of different variations on spreadsheets has been explored \cite{bock-2016-spreadsheet},
including spreadsheets with a more expressive formula language, user-defined functions and
static type checking.

As document-oriented programming system, spreadsheets have a very regular and limited
document structure, but it serves as a primary notation and an interface for both using and
programming. They are also interesting for their reactive evaluation.

\paragraph{Jupyter.}
Notebook systems for data science such as Jupyter \cite{kluyver-2016-jupyter} (Figure~\ref{fig:systems},
right) are also document-oriented programming systems. The document structure is a notebook, consisting
of a sequence of rich text blocks, code cells and program outputs. The notebook interface lets
the user edit the notebook, run computations, review the results and possibly also change
computation parameters interactively \cite{jupyter-2025-widgets}.

The document structure in notebook systems is a simple list of cells. Code included in the notebook
typically does not manipulate the document directly. It performs some computation and the
result of the computation is visualized by the notebook system using a table, chart or another
kind of custom visualization. In Jupyter, the runtime state is maintained by a background
kernel process and executed code snippets from the notebook modify the state. (An out-of-order
execution of cells can result in an inconsistent state, an issue that some systems aim to address
by automatic invalidation based on a dependency graph \cite{petricek-2018-wrattler}.)

In notebook systems, the user again works with a document, although they mostly edit code in an
embedded text-based code editor. The document state exists in a background process, which sets
Jupyter apart from other document-oriented programming systems discussed in this section.

~

\subsection{Current Research: Subtext and BootstrapLab}

The systems discussed in the previous section structure programs as documents and use the
document interface for their editing, but they do not use the document itself for structuring
code and for evaluation, an approach suggested by Boxer. A number of research projects aim to
explore this direction \cite{edwards-2005-subtext,petricek-2025-denicek,jakubovic-2022-bootstraplab}.

\paragraph{Subtext.}
Subtext is a series of research document-oriented programming systems \cite{edwards-2025-retro}
Most systems of the Subtext family use a document structured as a tree. Data and code are
also represented using a tree structure. For example, a conditional expression is a tree node
with two sub-trees for the two cases, similarly to how Boxer used nested boxes. Manipulating the
document is also the mechanism for using programs created using Subtext. A computation can be
triggered by updating an input value or through the graphical user interface and the result will
appear in the document.

Subtext experimented with a number of interesting aspects of the programming model and program
evaluation. In Subtext~1 \cite{edwards-2005-subtext}, the programming model is based on concrete
examples and copying. Programs are constructed by creating examples, constructing computations
with concrete values (as in spreadsheets). Code reuse is achieved by copying and modifyign
examples. In terms of technical dimensions \cite{jakubovic-2023-techdims}, Subtext's
\emph{Abstraction Construction} proceeds from concrete cases.

Another central theme in Subtext is the use of \emph{materialized execution}. Rather than reducing
a function call with a parameter to the result of the call, Subtext lets the user explore the
individual steps of the execution. The call node can be expanded to see a sub-tree containing
a copy of the function called, with the function parameter replaced with the actual argument.
Inspired by an earlier implementation of the mechanism \cite{edwards-2004-example},
Subtext 7 \cite{edwards-2017-reifying} also allowed users to modify the execution traces.
This makes it possible, for example, to debug test failures and see what alternative executions
would lead to the correct result.

Subtext~8~\cite{edwards-2018-directprog} introduced a different underlying representation of the
document. Rather than storing the document itself, the primary representation of the document
becomes a history of edit actions through which the document is modified. The representation erases
the distinction between program execution and user interaction. Whatever can be done by running
a program can also be done by the user through direct manipulation. A sequence of past actions
can also be lifted into a reusable entity, providing a mechanism for programming by demonstration.

Successors to Subtext including Baseline \cite{edwards-2025-baseline} and Denicek
\cite{petricek-2025-denicek} further refine the model based on edit history to support
collaborative development, schema change management and other user experiences.

\paragraph{BootstrapLab.}
BootstrapLab \cite{jakubovic-2022-bootstraplab} is a prototype document-oriented programming system
that explores how can the structured document representation be combined with self-sustainability.
This property, known from systems such as Smalltalk, refers to the ability to construct most
of the programming system using itself. This focus results in a number of interesting design
choices regarding the system's programming model and an approach to constructing the graphical
user interface.

BoostrapLab is based on a mutable document. The basic built-in programming mechanism is a
low-level assembly-like language that can be used to create programs that modify the document
tree structure. (Instructions in the language are ordered sub-nodes in the tree that are
executed one by one.) However, BootstrapLab then imagines using this low-level language to
create a high-level functional language with an interpreter that would work similarly to the
evaluation model of Subtext, i.e., by attaching the steps of the evaluation process, as well as
the final result, to the document tree. Unlike in Subtext, this functionality would not be
built-in feature of the system, but something built within the system, using its low-level
capabilities.

BootstrapLab also shows that the user interface for editing documents does not have to be
built-in as in most other document-oriented programming systems. To achieve this, BootstrapLab
treats a particular sub-tree of the document as a memory-mapped display containing a tree
structure that defines what the user sees as a tree-structured vector graphics akin to SVG.
BootstrapLab programs can modify the memory-mapped display to, for example, visually render a
part of the document tree and allow its editing through a tree view, reconstructing the graphical
interface known from Subtext.
...
\item \emph{Tree-Shaped Documents.} The tree can be displayed directly to user using
  ...
  The tree can be modelled after forms (Forms/3~\cite{burnett-2001-forms3}) or structured
  mixed-media documents such as HTML (Webstrates \cite{klokmose-2015-webstrates},
  Webnicek \cite{petricek-2025-denicek}), in which case the user can often view and edit
  the document directly (WYSIWYG) or through a source view (tree view widget).
...
\item \emph{Document Itself.} In many systems, their internal
  ...
  direct rendering of their document representation (Boxer~\cite{disessa-1986-boxer},
    HyperCard~\cite{atkinson-1987-hypercard}, Jupyter~\cite{kluyver-2016-jupyter}, Forms/3~\cite{burnett-2001-forms3}).
...
\item \emph{Edit History.} In a number of systems, the ground truth is not the document itself,
  ...
  (Subtext~8~\cite{edwards-2018-directprog}, Denicek~\cite{petricek-2025-denicek}, Baseline~\cite{edwards-2025-baseline,edwards-2021-typed}).
...
choice corresponds directly to the technical dimension \emph{factoring of complexity}.
...
\item \emph{Internal References.} A simpler mechanism for code reuse is a
  ...
  document location where it is referenced (the user typically sees the entity name).
  References can refer to functions (BootstrapLab~\cite{jakubovic-2022-bootstraplab}) or
  interactive components (Livelits~\cite{omar-2021-livelits}).

\item \emph{Managed Copy and Paste.} Several systems encourage users to reuse code using copy \&
  paste, but keep track of the copies (Subtext~\cite{edwards-2005-subtext},
  Baseline~\cite{edwards-2022-copy}, Forms/3~\cite{burnett-2001-forms3}, Denicek formulas
  \cite{petricek-2025-denicek}). A change made to a copy
  can be local (change of input), or propagate back to the source (fixing a bug).
...
\item \emph{Edit History Reference.} In systems where the underlying representation of a document
  is a history of edits (see \S\ref{sec:struct-repr}), it is also possible to specify behavior
  ...
  (Subtext~8~\cite{edwards-2005-subtext}, Denicek \cite{petricek-2025-denicek}).
\end{itemize}
...
kind of programming. However, our definition admits both end-user document systems with
limited programming capabilities and systems that can be fully reprogrammed from within themselves.
...
\item \emph{Imperative.} In the imperative programming model, code embedded in the document
  ...
  Webstrates~\cite{klokmose-2015-webstrates}, Subtext~8~\cite{edwards-2018-directprog}),
  while in others, commands can only modify state
...
\item \emph{Declarative.} In the declarative model, code in the document describes a computation
  that should be performed in order to obtain some result (Subtext~7~\cite{edwards-2017-reifying},
  Forms/3~\cite{burnett-2001-forms3}). The result may be displayed alongside
...
modify a set of registers, stored in a known location in the document tree (Figure~\ref{fig:blab} (a)).
The special instruction pointer register is a reference to the next instruction to be executed.
The low-level programming model includes operations such as key lookup, copying, and conditional jump.

A high-level programming model envisioned in BootstrapLab is the MASP language, modelled after
LISP (Figure~\ref{fig:blab} (b)), but using maps instead of lists. The document structure (nested
maps) represents the abstract syntax tree of functional-first programs. The code can be rendered
as a textual tree, but also visually (Figure~\ref{fig:blab} (c)). The MASP interpreter stores all
immediate information, as well as the final result, directly in the document (see~\S\ref{sec:eval-state}).
Although this was not fully implemented in the BootstrapLab prototype, a MASP interpreter or
compiler could be built using the low-level programming model.
...
\begin{wide}
  ...
  \vspace{0.5em}
  \includegraphics[scale=0.4]{fig/blab-instr.png}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.38\textwidth}\vspace{0pt}
  ...
  \vspace{0.5em}
  \includegraphics[scale=0.45]{fig/blab-masp.png}
  \end{minipage}
  \hfill
  ...
  \vspace{1em}
  \includegraphics[scale=0.45]{fig/blab-vis.png}
  \end{minipage}
\end{wide}
...
\item \emph{No Metaprogramming.} In many document-oriented programming systems, computation can
  ...
  Idyll \cite{conlen-2018-idyll}, Potluck \cite{litt-2022-potluck}, Forms/3~\cite{burnett-2001-forms3}).
...
\item \emph{Full Metaprogramming.} Only document-oriented programming systems where much of the
  ...
  (Ampleforth \cite{bracha-2022-ampleforth}, BootstrapLab \cite{jakubovic-2022-bootstraplab},
...
We consider these two aspects together as a signal design choice.
...
\item \emph{Naive Realism} vs. \emph{Hidden Representation.} In the naive realism model,
  ...
  Jupyter \cite{kluyver-2016-jupyter}, Subtext \cite{edwards-2005-subtext}, Forms/3~\cite{burnett-2001-forms3}).
...
\item \emph{Structured Representation} vs. \emph{Textual Representation.}
  ...
  AST using the same structure and edited using a structure editor (Boxer~\cite{disessa-1986-boxer},
...
\item \emph{Implicit Structure.} Document systems that let users programmatically manipulate
  ...
  (Webnicek~\cite{petricek-2025-denicek}, BootstrapLab~\cite{jakubovic-2022-bootstraplab}).
...
\item \emph{Explicit Structure.} In some systems, the document structure can be explicit
  ...
  research in programming systems (Subtext~10~\cite{edwards-2021-subtext10},
  Baseline~\cite{edwards-2021-typed}), but examples exist in document manipulation
  (XDuce \cite{hosoya-2003-xduce}, Ur/Web \cite{chlipala-2015-urweb}).
\end{itemize}
...
\item \emph{Renderer with Widgets.} In systems where the document representation is less
  ...
  Jupyter Widgets~\cite{jupyter-2025-widgets}, Forms/3~\cite{burnett-2001-forms3}).
...
\item \emph{In-System Editor.} Systems that provide sufficient expressive power and rendering
  ...
  design choices (Ampleforth \cite{bracha-2022-ampleforth}, BootstrapLab \cite{jakubovic-2022-bootstraplab},
...
The first choice is concerned with what triggers evaluation. The different choices have been
...
\item \emph{Implicit Trigger.} More immediate feedback may provided by automatically recomputing
  ...
  Ampleforth \cite{bracha-2025-example}, Forms/3~\cite{burnett-2001-forms3}).
...
\item \emph{Internalized Execution.} A document-oriented programming system can choose to use no
  ...
  computation terminates (BootstrapLab \cite{jakubovic-2022-bootstraplab}, Boxer~\cite{disessa-1986-boxer},
...
\item \emph{Materialized Execution.} Finally, a system can record every single step of the execution
  ...
  (Denicek \cite{petricek-2025-denicek}, Subtext~7~\cite{edwards-2017-reifying},
  BootstrapLab/MASP \cite{jakubovic-2022-bootstraplab}).
\end{itemize}

\begin{figure}[t]
\centering
\vspace{-1em}
\includegraphics[width=0.85\linewidth]{fig/stepper.png}
\vspace{-0.5em}
\caption{First six step of the execution of \texttt{foo 10 20} in the Boxer movie stepper.
  ...
  (1) The evaluation starts, (2) function definition replaces the
...
the Boxer movie stepper \cite{klotz-1989-boxer} makes it possible to see how the execution
...
where edits need to be merged or reconciled. Two systems that tackle this problem are
Baseline~\cite{edwards-2025-baseline} and Denicek \cite{petricek-2025-denicek}.

Consider a document with a root node \texttt{pioneers} containing two
...
the selected nodes. The design choice affects how the systems handle renaming.
...
One of the uses of the catalogue of design choices presented in this paper
(see \S\ref{sec:intro}) is to identify interesting under-explored designs for
document-oriented programming systems. We discuss four research directions that illustrate
such use of the catalogue.

\paragraph{Metaprogramming over Execution Traces.}
One of the choices for managing evaluation state (see \S\ref{sec:eval-state})
is to use \emph{Materialized Execution} where individual steps of the execution are recorded
as nodes in the document. This is only implemented in a few experimental systems
and an efficient implementation requires further research.
However, the model enables interesting capabilities, especially when combined with
metaprogramming capabilities (see~\S\ref{sec:prog-meta}) that make it possible for computations
inside the document to manipulate the document, i.e., the \emph{Document} or \emph{Full Metaprogramming}.

A system with the two features would make it possible to create programs that analyze the
execution of other programs embedded within the same document. This may be used for analysing
unit test failures (using a diff of the failing execution and the last successful execution),
to implement language-integrated provenance \cite{fahrenbach-2019-provenance} or to collect
information for automatically generating linked data visualizations \cite{perera-2022-linked}.

\paragraph{Memory-Mapped Display.}
A design choice that has been explored briefly in BootstrapLab~\cite{jakubovic-2022-bootstraplab}
is the use of \emph{Memory-Mapped Display} as the display mechanism (see~\S\ref{sec:gui-display}).
The choice requires programming systems where computations can modify the document,
i.e., support \emph{Document} or \emph{Full Metaprogramming} capabilities (see~\S\ref{sec:prog-meta}).

The choice makes it possible to build document-oriented programming systems where individual programs
have full control of what the user sees. The design choice is akin to the design of the Smalltalk
programming environment, which is itself implemented in Smalltalk and uses the primitive graphics
API of the runtime with reflection in order to build tools such as the class browser \cite{goldberg-1984-orange}.
The combination would make it possible to build a document-based equivalent of the Smalltalk design.

\paragraph{Unifying Execution and Interaction.}
A theme that has been suggested by Subtext~8~\cite{edwards-2018-directprog} is unifying
program execution with user interaction. The combination of design choices that enables this
is the use of \emph{Edit History} as the document representation (see~\S\ref{sec:struct-repr})
together with \emph{Internalized} or \emph{Materialized Execution} evaluation state
(see~\S\ref{sec:eval-state}). The combination means that evaluating code in a document
generates edits that modify the document that are indistinguishable from edits that could be
performed by the user through a graphical interface.
The combination could enable a range of programming by demonstration user experiences
where reusable programs are constructed by enacting the computation using sample concrete
values as in Pygmalion~\cite{smith-1977-pygmalion}.

\paragraph{Variations on Spreadsheets.}
Spreadsheet systems such as Excel can be characterized by using the \emph{Two-Dimensional Grid}
as the document shape (see \S\ref{sec:struct-shape}), \emph{Declarative} programming model
(see \S\ref{sec:prog-model}) with \emph{Implicitly Triggered} evaluation (\S\ref{sec:eval-mode})
and the support for \emph{Absolute} and \emph{Relative References}
as the node identification mechanism~(\S\ref{sec:eval-ident}).

Interesting programming systems can be imagined by varying one of the design choices.
One variation is to generalize the document shape from a regular two-dimensional grid to
other structures. This combination has been partly explored by Forms/3 \cite{burnett-2001-forms3}
and recently Nezt~\cite{bazant-2018-nezt}, but there are likely multiple other variations.
It may also be interesting to generalize the node identification mechanism to arbitrary
\emph{Selectors}, allowing the users to construct formulas that use more general queries
to collect data from the spreadsheet.

Another variation on spreadsheets would be to use the \emph{Edit History} choice of
document representation (see \S\ref{sec:struct-repr}). The combination would enable some of the
affordances of Subtext and Baseline \cite{edwards-2025-baseline,edwards-2018-directprog}.
It could be possible to construct computations by interactively manipulating concrete values
and capturing the edit history as a formula. It could also be possible to define merging of edit
histories and address schema change challenges arising from diverging versions of
spreadsheets~\cite{edwards-2025-schema,burnett-2014-frieda}.

\section{Related work}

The history of document-oriented programming systems discussed in this paper has
inevitably been selective. The papers referenced as examples are generally
pioneering historical systems, widely-used systems, or current research projects.

The idea of \emph{documents as user interfaces} \cite{bier-1991-docs} focused on user interaction
rather than programming, but is closely related to document-oriented programming systems. It
inspired research on active documents \cite{quint-1994-active} where document editor can include
logic for collaborative editing, annotating documents or managing files on disk. The logic in those
systems, however, is not a part of the document. EmbeddedButtons \cite{bier-1991-buttons} attaches
the logic to buttons that can be included in documents, copied and shared.

Despite our definition (see \S\ref{sec:def}), the boundary of what
qualifies as a document-oriented programming system remains fuzzy. There are several related
systems and formal models that we choose not to include as examples. The document calculus
\cite{crichton-2024-document} captures systems for generating (static) documents, but it does
not model interactive documents. We included the Hazel structure editor \cite{omar-2017-hazelnut}
as an example of a system with widgets (livelits) \cite{omar-2021-livelits} and also because of the
notebook vision presented as Planet Hazel~\cite{bandukwala-2024-planet}, but excluded structure
editors \cite{teitelbaum-1981-cps} and projectional editors \cite{voelter-2014-projection}, even
if they can be used for editing programs with projection widgets akin to
livelits~\cite{mcnutt-2023-projectional}.

There are also multiple programming systems that have some of the characteristics of
document-oriented programming systems, but not all. RenkonPad~\cite{ohshima-2025-renkonpad}
is based on the DOM programming model, but uses it to build a window-based interface.
Systems built on top of Webstrates~\cite{radle-2017-codestrates,klokmose-2019-video} are
also relevant, but do not fundamentally change the underlying model.
Varv~\cite{borowski-2022-varv} and Infusion~\cite{basman-2025-infusion} are closely related,
but do not view program as a document. More historically, the UI2 or Morphic
\cite{ungar-2007-self,maloney-1995-morphic} user interface in Self has the directness and
liveness of many document-oriented programming systems, but does not directly use the document metaphor.

Methodologically, our work builds on Technical Dimensions of Programming
Systems~\cite{jakubovic-2023-techdims}. We ``zoom in'' to examine a more narrow space
and uncover domain-specific design choices. The potential uses of our paper are also similar and
include comparison of existing systems and exploration of a design space. Our aim is to
map design space, rather than provide a comprehensive literature review and so our choice of
example systems is selective. Using a rigorous literature review methodology, as for example
in the review of exploratory and live programming and coding~\cite{rein-2019-exploratory}
would be interesting, but possibly more difficult due to the lack of established terminology.

\section{Conclusion}
Document-oriented programming systems have been around in various forms and constitute an
interesting research field, but systems in this area often exist in isolation.
By defining the notion of a \emph{document-oriented programming system}, we aim to highlight
an appealing programming paradigm that has been around for a long time, but remained somewhat
out of sight.
%
Programming systems in this category share a number of characteristics, but also differ in
important ways. Our catalogue of design choices maps the design
space of document-oriented programming systems. It provides a basis for comparing existing
systems and envisioning new research directions to explore previously under-explored combinations
of design choices.

Many of the document-oriented programming systems discussed in this paper pursue ambitious goals,
such as making programming more transparent, more accessible to non-experts, and supporting gradual
progression from an end-user to a developer. In fact, systems such as HyperCard achieved those goals
in a specific context! We hope that our work contributes to better understanding and supports
research on programming systems that share these aspirations.
